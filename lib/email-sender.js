 ...
 */
export async function dispatchEmail({ userId, from, to, subject, message, html, quotedText, isPlainText }) {
  ...
  // Generate HTML if not in plain text mode
  const fullHtml = isPlainText
    ? null
    : (html || buildColorfulEmailHtml({
        from: cleanFrom,
        to: cleanTo,
  ...
  if (targetInbox) {
    await Email.create({
      ...
      from: cleanFrom,
      subject: cleanSubject,
      bodyHtml: fullHtml || `<div style="white-space: pre-wrap; font-family: sans-serif; padding: 16px;">${fullText}</div>`,
      bodyText: fullText,
      isRead: false,
    });
